...
public class RF2ExtractionService {
	...
	public void extractConcepts(InputStream rf2SnapshotArchive, Set<Long> conceptIds, Map<Long, String> refsetsToInclude, File outputDirectory, String effectiveTime)
			throws ReleaseImportException, IOException {
		logger.info("Extracting {} concepts from RF2.", conceptIds.size());
		ReleaseImporter releaseImporter = new ReleaseImporter();
		try (RF2ExtractionWriter extractionWriter = new RF2ExtractionWriter(conceptIds, refsetsToInclude, effectiveTime, outputDirectory);
			 InputStream testSubontologyModuleZip = getClass().getResourceAsStream("/SnomedCT_TestSubonologyModule.zip")) {

			releaseImporter.loadEffectiveSnapshotReleaseFileStreams(Set.of(rf2SnapshotArchive, testSubontologyModuleZip), LoadingProfile.complete, extractionWriter, false);
		}
